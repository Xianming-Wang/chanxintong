...
class Shuini3Spider(scrapy.Spider):
    name = 'gangtie_1'
    start_urls = [
        'http://www.cnmeti.com/zxzx/c11_cp1/'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gangtie_1.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gangtie_1'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gangtie_1'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            for node in odd_list:
                try:
                    self.items['task_name'] = '钢铁A-5-冶金工业网'
                    content_url = 'http://www.cnmeti.com' + node.xpath('./a/@href').extract()[0]
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                self.items['origin_source'] = re.xpath('//div[@class="title-info clearfix"]/span[3]/em/text()')[0]
