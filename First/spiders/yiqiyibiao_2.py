...
class Shuini3Spider(scrapy.Spider):
    name = 'yiqiyibiao_2'
    start_urls = [
        'http://www.caia.org.cn/erJiList/62.html?cpage=1&childrenId=65'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('yiqiyibiao_2.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'yiqiyibiao_2'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('yiqiyibiao_2'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="news-container"]/a')
            for node in odd_list:
                try:
                    self.items['task_name'] = '机械仪器仪表A-10-中国分析测试协会'
                    content_url = 'http://www.caia.org.cn/' + node.xpath('./@href').extract()[0]
                    self.items['content_url'] = content_url
                    title = ''.join(node.xpath('./div/text()').extract())
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                next_page_num = int(response.url.split('cpage=')[-1].split('&')[0]) + 1
                next_page_url = 'http://www.caia.org.cn/erJiList/62.html?cpage={}&childrenId=65'.format(next_page_num)
    ...
    def details_page(self, content_url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = res.xpath('//div[@class="uedit-container"]')
                ...
                tx = res.xpath('//div[@class="col-lg-9 letf huise"]/text()')[0]
                self.items['origin_source'] = tx.replace('文章来源：','')
