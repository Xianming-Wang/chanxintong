...
class Shuini3Spider(scrapy.Spider):
    name = 'gangtie_4'
    start_urls = [
        'https://www.zh818.com/Get/hongguan/index_1.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gangtie_4.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gangtie_4'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gangtie_4'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@id="content2010-text"]/table/tr')
            for node in odd_list:
                try:
                    self.items['task_name'] = '钢铁A-22-钢材价格网'
                    content_url = 'https://www.zh818.com' + node.xpath('./td[1]/a/@href').extract()[0]
                    self.items['content_url'] = content_url
                    title = node.xpath('./td[1]/a/@title').extract()[0]
                    self.items['title'] = title.strip()
                    publish_date = node.xpath('./td[2]/span/text()').extract()[0]
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                next_page_num = int(str(response.url).split('_')[-1].replace('.html', '')) + 1
                next_page_url = 'https://www.zh818.com/Get/hongguan/index_{}.html'.format(next_page_num)
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@class="content-text1"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//span/p/text()')).strip()
                self.items['origin_source'] = re.xpath('//div[@id="content-copyright"]/a[2]/text()')[0]
