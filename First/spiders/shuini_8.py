...
class Shuini3Spider(scrapy.Spider):
    ...
    start_urls = [
        'http://eps.qlssn.com/BasicData/Site/NewsList/1?newsGroupId=201',
        'http://eps.qlssn.com/HomeSite/Site/NewsList?newsGroupId=101'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('shuini_8.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'shuini_8'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('shuini_8'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="box"]/ul[@class="newslist"]/li')
            for node in odd_list:
                try:
                    if response.url.find('201') != -1:
                        self.items['task_name'] = '水泥A-10-祁连山电子采购平台'
                    else:
                        self.items['task_name'] = '水泥A-9-祁连山电子采购平台'
                    content_url = 'http://eps.qlssn.com' + node.xpath('./a/@href').extract()[0]
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                next_page_num = int(str(response.url).split('?')[0].split('/')[-1]) + 1
                next_page_url = str(response.url).split('/')[:-1] + str(next_page_num) + '?' + str(response.url).split('?')[-1]
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//section[@id="divContent"]')
