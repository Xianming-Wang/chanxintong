...
class Shuini3Spider(scrapy.Spider):
    name = 'jidian'
    start_urls = [
        'http://www.cccme.org.cn/news/channel-10---1.aspx'
    ]
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//ul[@id="newslist"]/li')
            for node in odd_list:
                try:
                    self.items['task_name'] = '机电A-12-中国机电产品出口商会'
                    content_url = 'http://www.cccme.org.cn' + node.xpath('./a/@href').extract()[0]
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@id="fontsize"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//div[@id="fontsize"]/p/text()')).strip()
                self.items['origin_source'] = re.xpath('//h2[@class="new-info-buj m-bottom-20"]/span/text()')[0].replace('来源:','')
