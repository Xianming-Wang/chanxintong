...
class Shuini3Spider(scrapy.Spider):
    name = 'nengyuan'
    start_urls = [
        'https://newenergy.in-en.com/finance/Windpower/list1065-1.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('nengyuan.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'nengyuan'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('nengyuan'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            for node in odd_list:
                try:
                    self.items['task_name'] = '能源A-262-国际新能源网'
            ...
            if self.turn_page:
                ...
                next_page_url = 'https://newenergy.in-en.com/finance/Windpower/list1065-{}.html'.format(next_page_num)
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@class="rightDetail fr"]')
