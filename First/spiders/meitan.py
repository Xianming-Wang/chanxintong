...
class Shuini3Spider(scrapy.Spider):
    name = 'meitan'
    start_urls = [
        'https://www.china5e.com/coal/general/index_1.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('meitan.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'meitan'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('meitan'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="bd"]/ul')
            for node in odd_list:
                try:
                    self.items['task_name'] = '煤炭A-44-5e中国能源网'
                    content_url = node.xpath('./li/a/@href').extract()[0]
                    self.items['content_url'] = content_url
                    title = node.xpath('./li/a/text()').extract()[0]
                    self.items['title'] = title.strip()
                    publish_date = node.xpath('./li/span/text()').extract()[0]
            ...
            if self.turn_page:
                ...
                next_page_url = 'https://www.china5e.com/coal/general/index_{}.html'.format(next_page_num)
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                self.items['origin_source'] = re.xpath('//div[@class="showtitinfo"]/text()')[-1].strip()
