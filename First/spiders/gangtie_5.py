...
class Shuini3Spider(scrapy.Spider):
    name = 'gangtie_5'
    start_urls = [
        'http://news.zgw.com/hyjj/page_1.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gangtie_5.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gangtie_5'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gangtie_5'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="trends"]')
            for node in odd_list:
                try:
                    self.items['task_name'] = '钢铁A-19-中钢网'
            ...
            if self.turn_page:
                ...
                next_page_url = 'http://news.zgw.com/hyjj/page_{}.html'.format(next_page_num)
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@class="left_content"]/div[@class="neirong"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//div[@class="left_content"]/div[@class="neirong"]/p/text()')).strip()
                self.items['origin_source'] = re.xpath('//div[@class="fenxiang"]/p/text()')[0].replace('来源：', '').strip()
