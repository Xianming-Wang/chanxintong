...
class Shuini3Spider(scrapy.Spider):
    name = 'nengyuan_4'
    start_urls = [
        'https://solar.in-en.com/corp/list1078-1.html',
        'https://solar.in-en.com/news/list1059-1.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('nengyuan_4.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'nengyuan_4'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('nengyuan_4'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            for node in odd_list:
                try:
                    if response.url.find('corp') != -1:
                        self.items['task_name'] = '能源A-128-国际太阳能光伏网'
                    else:
                        self.items['task_name'] = '能源A-125-国际太阳能光伏网'
