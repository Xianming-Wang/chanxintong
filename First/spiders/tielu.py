...
class Shuini3Spider(scrapy.Spider):
    name = 'tielu'
    start_urls = [
        'http://www.crecgec.com/forum.php?mod=forumdisplay&fid=2&sortid=14&filter=sortid&sortid=14&page=1',
        'http://www.crecgec.com/forum.php?mod=forumdisplay&fid=2&sortid=12&filter=sortid&sortid=12&page=1'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('tielu.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'tielu'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('tielu'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('///form/li')
            for node in odd_list:
                try:
                    if response.url.find('sortid=14') != -1:
                        self.items['task_name'] = '铁路建设A-78-中铁鲁班商务网'
                    else:
                        self.items['task_name'] = '铁路建设A-77-中铁鲁班商务网'
                    content_url = 'http://www.crecgec.com/' + node.xpath('./a/@href').extract()[0]
                    self.items['content_url'] = content_url
                    title = node.xpath('./a/@title').extract()[0]
                    self.items['title'] = title.strip()
                    publish_date = node.xpath('./a/em/text()').extract()[0]
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                next_page_num = int(str(response.url).split('page=')[-1]) + 1
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@class="allNoticCont"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//div[@class="allNoticCont"]/p/text()')).strip()
