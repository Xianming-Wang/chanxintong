# -*- coding: utf-8 -*-
import scrapy, os, html
from First.items import FirstItem
from configparser import ConfigParser
import time, requests
from lxml import etree

class Shuini3Spider(scrapy.Spider):
    name = 'shuini_4'
    # allowed_domains = ['http://www.zybtp.com']
    start_urls = ['https://cucc.go.1688.com/app/caigouportal_purchaseList.htm?site_id=caigouportal&'
                  'site_key=2b59679705d94336b270078103b91494&page_type=purchase&region_type=big&'
                  'segment_id=site_content&layout_type=m0s5&app_key=db605a4cd551471ca09b5af4da13714f&'
                  'is_diy=false&curpage=1&is_compact=false&include_completed=false&pagesize=100&'
                  'csrf_token=edf69ed87cfa2f949142df9cbad8ed95&biz_type=buyoffer&subject=&_input_charset=utf-8']

    def __init__(self):
        self.turn_page = True
        self.logger.info('shuini_4.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'shuini_4'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('shuini_4'), 'r') as f:
                self.url_list = f.read().splitlines()
        except Exception as e:
            self.url_list = []

    def parse(self, response):
        self.items['list_url'] = response.url
        if response.status == 200:
            self.logger.info('采集的列表页URL %s', response.url)
            odd_list = response.xpath('//div[@class="mod-list "]/div[@class="row fd-clr odd"]')
            # clr_list = response.xpath('//div[@class="mod-list "]/div[@class="row fd-clr "]')
            for node in odd_list:
                try:
                    self.items['task_name'] = '水泥A-5-1688大企业采购'
                    content_url = node.xpath('.//div[@class="s s20 "]/h3[@data-version=""]/a/@href').extract()[0]
                    self.items['content_url'] = content_url
                    title = node.xpath('.//div[@class="s s20 "]/h3[@data-version=""]/a/text()').extract()[0]
                    self.items['title'] = title.strip()
                    mon_day = node.xpath('./div/div/text()').extract()[0]
                    year = node.xpath('./div/div/span/text()').extract()[0]
                    publish_date = str(year + '-'+ mon_day.replace('.', '-')).strip()
                    self.items['publish_date'] = publish_date
                except Exception as e:
                    self.items['detail_url'] = ''
                    self.items['status'] = '采集失败'
                    self.items['error'] = '列表页解析失败'
                    self.logger.error('列表页内容解析错误 报错信息为： %s' % e)
                    continue

                current = self.time_now()
                if self.task_filter(current, publish_date):
                    self.details_page(content_url)
                    yield self.items
                    self.turn_page = False
                    continue
            if self.turn_page:
                next_page_num = int(str(response.url).split('curpage=')[-1].split('&')[0]) + 1
                next_page_url = 'https://cucc.go.1688.com/app/caigouportal_purchaseList.htm?site_id=caigouportal&' \
                                'site_key=2b59679705d94336b270078103b91494&page_type=purchase&region_type=big&' \
                                'segment_id=site_content&layout_type=m0s5&app_key=db605a4cd551471ca09b5af4da13714f&' \
                                'is_diy=false&curpage={}&is_compact=false&include_completed=false&pagesize=100&' \
                                'csrf_token=edf69ed87cfa2f949142df9cbad8ed95&biz_type=buyoffer&subject=&' \
                                '_input_charset=utf-8'.format(next_page_num)
                yield scrapy.Request(next_page_url, callback=self.parse)
        else:
            self.items['detail_url'] = ''
            self.items['status'] = '采集失败'
            self.items['error'] = '列表页采集失败'

    def time_now(self):
        current = time.localtime(time.time())
        return current

    def task_filter(self, current, publish_date):
        p_date = time.strptime(publish_date, '%Y-%m-%d')
        if current.tm_mon == p_date.tm_mon and current.tm_mday == p_date.tm_mday \
                and self.items['content_url'] not in self.url_list:
            return True
        else:
            return False

    def details_page(self, url):
        header = {
            'User-Agent': self.config.get('header', 'user_agent'),
            'Accept': self.config.get('header', 'accept'),
            'Accept-Language': self.config.get('header', 'accept_language')
        }

        response = requests.get(url, headers=header)
        self.items['detail_url'] = response.url
        if response.status_code == 200:
            re = etree.HTML(response.text)  # 为什么不能用content而是text
            try:
                content_html = re.xpath('//div[@id="container"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//ul[@class="detail-property__list"]/li/span/text()')).replace('\xa0', '')
                self.items['origin_source'] = ''
                self.items['origin_author'] = ''
                self.items['update_date'] = int(time.time())
                self.items['title_in_content'] = ''
                self.items['status'] = '采集成功'
                self.items['error'] = ''
                self.logger.info('采集的items数据：%s' % self.items)
            except Exception as e:
                self.items['status'] = '采集失败'
                self.items['error'] = '详情页解析失败'
                self.logger.error('详情页解析错误，详情页URL：%s' % url)
                self.logger.error('错误详情：%s' % e)
        else:
            self.items['status'] = '采集失败'
            self.items['error'] = '详情页采集失败'


