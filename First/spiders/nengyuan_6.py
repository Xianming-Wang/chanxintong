...
class Shuini3Spider(scrapy.Spider):
    name = 'nengyuan_6'
    start_urls = [
        'https://www.china5e.com/new-energy/clean-energy-vehicles/index_1.html',
        'https://www.china5e.com/new-energy/stored-energy/index_1.html',
        'https://www.china5e.com/new-energy/wind-energy/index_1.html',
        'https://www.china5e.com/new-energy/solar-power/index_1.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('nengyuan_6.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'nengyuan_6'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('nengyuan_6'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//ul[@class="lista maring10"]/li')
            for node in odd_list:
                try:
                    if response.url.find('clean-energy-vehicles'):
                        self.items['task_name'] = '能源A-17-中国能源网'
                    elif response.url.find('stored-energy'):
                        self.items['task_name'] = '能源A-15-中国能源网'
                    elif response.url.find('wind-energy'):
                        self.items['task_name'] = '能源A-12-中国能源网'
                    else:
                        self.items['task_name'] = '能源A-11-中国能源网'
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@style="min-height:300px;"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//div[@style="min-height:300px;"]/p/text()')).strip()
                tx = re.xpath('//div[@class="showtitinfo"]/text()')[-1]
                self.items['origin_source'] = tx.strip()
