...
class Shuini3Spider(scrapy.Spider):
    name = 'gonggongziyuan_2'
    start_urls = [
        'http://ggzyjy.nmg.gov.cn/jyxx/zfcg/cggg?currentPage=1',
        'http://ggzyjy.nmg.gov.cn/jyxx/jsgcZbgg?currentPage=1'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gonggongziyuan_2.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gonggongziyuan_2'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gonggongziyuan_2'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//tr')
            for node in odd_list[1:]:
                try:
                    if response.url.find('zfcg') != -1:
                        self.items['task_name'] = '公共资源交易A-161-内蒙古'
                    else:
                        self.items['task_name'] = '公共资源交易A-159-内蒙古'
                    content_url = 'http://ggzyjy.nmg.gov.cn' + node.xpath('./td[3]/a/@href').extract()[0]
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@class="detail_contect"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//div[@class="detail_contect"]/div/*/text()')).strip()
