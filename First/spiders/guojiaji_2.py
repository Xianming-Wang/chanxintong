...
class Shuini3Spider(scrapy.Spider):
    name = 'guojiaji_2'
    start_urls = [
        'http://www.ctba.org.cn/xwzx.jsp?offset=0&&tn=XMXX&defaultClickValue='
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('guojiaji_2.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'guojiaji_2'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('guojiaji_2'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="list-group"]//li')
            for node in odd_list:
                try:
                    self.items['task_name'] = '国家级URL-59-中国招投标协会A '
                    code = node.xpath('./div/a/@onclick').extract()[0].replace('info(','').replace(')','')
                    content_url = 'http://www.ctba.org.cn/list_show.jsp?record_id={}'.format(code)
                    self.items['content_url'] = content_url
                    title = node.xpath('./div/a/@title').extract()[0]
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                next_page_num = int(response.url.split('offset=')[-1].split('&')[0]) + 25
                next_page_url = response.url.split('offset=')[0] + 'offset={}'.format(next_page_num) + response.url.split('offset=')[-1]
    ...
    def details_page(self, content_url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = res.xpath('//div[@class="bd"]')
