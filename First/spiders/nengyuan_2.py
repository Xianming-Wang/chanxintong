...
class Shuini3Spider(scrapy.Spider):
    name = 'nengyuan_2'
    start_urls = [
        'https://oil.in-en.com/news/intl/list128-1.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('nengyuan_2.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'nengyuan_2'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('nengyuan_2'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            for node in odd_list:
                try:
                    self.items['task_name'] = '能源A-149-国际石油网'
            ...
            if self.turn_page:
                ...
                next_page_url = 'https://oil.in-en.com/news/intl/list128-{}.html'.format(next_page_num)
