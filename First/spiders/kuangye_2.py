...
class Shuini3Spider(scrapy.Spider):
    name = 'kuangye_2'
    start_urls = [
        'http://www.kyjyw.com/dxnews/scan_news.aspx',
        'http://www.kyjyw.com/price/scan_price.aspx'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('kuangye_2.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'kuangye_2'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('kuangye_2'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//tr[@style="color:#333333;background-color:White;"]')
            for node in odd_list[1:]:
                try:
                    if response.url.find('dxnews') != -1:
                        self.items['task_name'] = '矿业A-25-矿业交易网'
                    else:
                        self.items['task_name'] = '矿业A-55-矿业交易网'
                    content_url = 'http://www.kyjyw.com/dxnews/' + node.xpath('./td[2]/a/@href').extract()[0]
                    self.items['content_url'] = content_url
                    title = node.xpath('./td[2]/a/text()').extract()[0]
