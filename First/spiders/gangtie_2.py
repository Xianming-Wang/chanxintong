...
class Shuini3Spider(scrapy.Spider):
    name = 'gangtie_2'
    start_urls = [
        'https://gangchang.lgmi.com/gc_FF3A1_list.htm'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gangtie_2.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gangtie_2'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gangtie_2'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="lf_bm"]/ul/li')
            for node in odd_list:
                try:
                    self.items['task_name'] = '钢铁A-36-兰格钢铁网'
                    ...
                    publish_date = '-'.join(publish_date[1:-1].split('.'))
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                next_page_num = int(str(response.url).split('c11_cp')[-1].replace('/', '')) + 1
                next_page_url = 'http://www.cnmeti.com/zxzx/c11_cp{}/'.format(next_page_num)
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = res.xpath('//table[@width="100%"]')
