...
class Shuini3Spider(scrapy.Spider):
    name = 'gonggongziyuan_3'
    start_urls = [
        'http://www.hljggzyjyw.gov.cn/trade/tradezfcg?cid=E92C3437-9CB8-40FB-BC5C-E78A6E4E475D&pageNo=1'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gonggongziyuan_3.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gonggongziyuan_3'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gonggongziyuan_3'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="right_box"]/ul/li')
            for node in odd_list:
                try:
                    self.items['task_name'] = '公共资源交易A-106-黑龙江省'
                    content_url = 'http://www.hljggzyjyw.gov.cn' + node.xpath('./a/@href').extract()[0]
                    self.items['content_url'] = content_url
                    title = node.xpath('./a/text()').extract()[-1]
                    self.items['title'] = title.strip()
                    publish_date = node.xpath('./span[3]/text()').extract()[0].strip()
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@id="contentdiv"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//div[@id="contentdiv"]/p/*/text()')).strip()
                self.items['origin_source'] = re.xpath('//div[@class="nav-line"]/span[2]/text()')[0].replace('来源：','')
