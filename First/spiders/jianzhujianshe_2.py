...
class Shuini3Spider(scrapy.Spider):
    name = 'jianzhujianshe_2'
    start_urls = [
        'http://www.abbs.com.cn/news/index.php?cate=3&page=1&query='
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('jianzhujianshe_2.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'jianzhujianshe_2'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('jianzhujianshe_2'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            date_list = response.xpath('//td[@valign="top"]/b/text()').extract()
            link = response.xpath('//td[@valign="top"]/a/@href').extract()
            title = response.xpath('//td[@valign="top"]/a/text()').extract()
            for node in range(len(date_list)):
                try:
                    self.items['task_name'] = '建筑建设A-85-ABBS建筑论坛'
                    content_url = 'http://www.abbs.com.cn/news/' + link[node]
                    self.items['content_url'] = content_url
                    title = title[node]
                    self.items['title'] = title.strip()
                    publish_date = date_list[node]
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                next_page_num = int(str(response.url).split('page=')[-1].split('&')[0]) + 1
                next_page_url = 'http://www.abbs.com.cn/news/index.php?cate=3&page={}&query='.format(next_page_num)
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//td[@valign="top"]//td[@class="s"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('////td[@valign="top"]//td[@class="s"]/text()')).strip()
