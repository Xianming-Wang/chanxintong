...
class Shuini3Spider(scrapy.Spider):
    name = 'gangtie_3'
    start_urls = [
        'http://www.coal.com.cn/Coal_NewsList_1_06860688_.htm'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gangtie_3.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gangtie_3'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gangtie_3'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            for node in odd_list:
                try:
                    self.items['task_name'] = '钢铁A-26-煤炭网'
                    content_url = 'http://www.coal.com.cn' + node.xpath('./a/@href').extract()[0]
            ...
            if self.turn_page:
                ...
                next_page_url = 'http://www.coal.com.cn/Coal_NewsList_{}_06860688_.htm'.format(next_page_num)
