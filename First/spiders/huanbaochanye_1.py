...
class Shuini3Spider(scrapy.Spider):
    name = 'huanbaochanye_1'
    start_urls = [
        'http://www.epwho.com/news/list_1_3585.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('huanbaochanye_1.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'huanbaochanye_1'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('huanbaochanye_1'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="hb_list"]/ul/li')
            for node in odd_list:
                try:
                    self.items['task_name'] = '环保产业A-33-第一环保网'
                    content_url = node.xpath('./h1/a/@href').extract()[0]
                    self.items['content_url'] = content_url
                    title = node.xpath('./h1/a/text()').extract()[0]
                    self.items['title'] = title.strip()
                    publish_date = node.xpath('./div/div/span[2]/text()').extract()[0]
                    publish_date = publish_date.replace('时间：','').strip()
                    self.items['publish_date'] = publish_date
                    origin_source = node.xpath('./div/div/span[2]/text()').extract()[0]
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                next_page_num = int(str(response.url).split('_')[1]) + 1
                next_page_url = response.url.split('_')[0] + '_{}_.html'.format(next_page_num)
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                self.items['pure_content'] = ''.join(re.xpath('//div[@class="content"]/p/span/text()')).strip()
