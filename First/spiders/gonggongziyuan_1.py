...
class Shuini3Spider(scrapy.Spider):
    name = 'gonggongziyuan_1'
    start_urls = [
        'http://ggzy.jiangxi.gov.cn/web/jyxx/002001/1.html'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gonggongziyuan_1.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gonggongziyuan_1'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gonggongziyuan_1'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//div[@class="ewb-infolist"]/ul/li')
            for node in odd_list:
                try:
                    self.items['task_name'] = '公共资源交易A-165-江西省'
                    content_url = 'http://ggzy.jiangxi.gov.cn' + node.xpath('./a/@href').extract()[0]
            ...
            if self.turn_page:
                ...
                next_page_url = '/'.join(response.url.split('/')[:-1]) + '/{}.html'.format(next_page_num)
