...
class Shuini3Spider(scrapy.Spider):
    name = 'gonggongziyuan_6'
    start_urls = [
        'https://www.hnsggzy.com/jygkzfcg/index_1.jhtml'
    ]

    def __init__(self):
        self.turn_page = True
        self.logger.info('gonggongziyuan_6.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'gonggongziyuan_6'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('gonggongziyuan_6'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            for node in odd_list:
                try:
                    self.items['task_name'] = '公共资源交易A-50-湖南省'
                    ...
                    origin_source = node.xpath('./div[2]/div/text()').extract()[0].replace('来源：','')
                    self.items['origin_source'] = origin_source
                except Exception as e:
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//table[@class="gycq-table"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//table[@class="gycq-table"]//p/*/*/text()'))
