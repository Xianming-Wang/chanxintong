...
class Shuini3Spider(scrapy.Spider):
    ...
    start_urls = ['http://bid.gzmtr.com/bizzbgg/index.html']

    def __init__(self):
        self.turn_page = True
        self.logger.info('guidao.py的日志')
        self.config = ConfigParser()
        self.config.read(os.getcwd() + './spiders/config/config.ini')
        self.items = FirstItem()
        self.items['spider_name'] = 'guidao'
        try:
            with open(os.getcwd() + '/spiders/url_deduplication/{}.txt'.format('guidao'), 'r') as f:
    ...
    def parse(self, response):
        ...
        if response.status == 200:
            ...
            odd_list = response.xpath('//ul[@class="list"]/li')
            for node in odd_list:
                try:
                    self.items['task_name'] = '轨道交通A-71-广州地铁'
                    content_url = 'http://bid.gzmtr.com/bizzbgg/' + str(node.xpath('./a/@href').extract()[0]).replace('./','')
                ...
                if self.task_filter(current, publish_date):
                    ...
                    continue
            if self.turn_page:
                page = str(response.url).split('_')[-1].split('.')[0]
                if page:
                    next_page_num = int(page) + 1
                    next_page_url = 'http://bid.gzmtr.com/bizzbgg/index_{}.html'.format(next_page_num)
                else:
                    next_page_url = 'http://bid.gzmtr.com/bizzbgg/index_1.html'
    ...
    def details_page(self, url):
        ...
        if response.status_code == 200:
            ...
            try:
                content_html = re.xpath('//div[@class="section_main"]')
                html_content = etree.tostring(content_html[0]).decode('utf-8')
                self.items['html_content'] = html.unescape(html_content)
                self.items['pure_content'] = ''.join(re.xpath('//div[@class="TRS_Editor"]/div/text()')).strip()
